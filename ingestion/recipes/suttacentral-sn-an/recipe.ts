...
function matchesPinnedSource(raw: Uint8Array, source: SourceFile): boolean {
  if (raw.byteLength === source.byteSize && sha(raw) === source.sha256) return true
  const text = new TextDecoder('utf8', { fatal: true }).decode(raw)
  if (text.includes('\r\n')) return false
  const upstreamCrlf = new TextEncoder().encode(text.replaceAll('\n', '\r\n'))
  return upstreamCrlf.byteLength === source.byteSize && sha(upstreamCrlf) === source.sha256
}
...
async function parseManifest(bytes: Uint8Array, location: string): Promise<Parsed> {
  ...
  for (const source of manifest.files) {
    ...
    if (!matchesPinnedSource(raw, source)) throw new Error(`Checksum mismatch ${source.file}`)
    const values = JSON.parse(raw.toString('utf8')) as Record<string, string>
    const ids = [...new Set(Object.keys(values).map((key) => key.split(':', 1)[0]).filter((id) => /^(sn|an)\d+(?:\.\d+)+$/u.test(id)))]
    for (const localId of ids) {
      const collection = localId.startsWith('sn') ? 'sn' : 'an' as Collection
      const entries = Object.entries(values).filter(([id, text]) => id.startsWith(`${localId}:`) && text !== '')
      if (!entries.length) continue
      const title = (values[`${localId}:0.2`] ?? values[`${localId}:0.1`] ?? localId).trim()
      if (byId.has(localId)) throw new Error(`Duplicate SN/AN sutta ${localId}`)
      byId.set(localId, { collection, localId, title, segments: entries.map(([id, text]) => ({ id: id.slice(localId.length + 1), text })) })
    }
  }
  const suttas = [...byId.values()].sort((a, b) => a.collection.localeCompare(b.collection) || a.localId.localeCompare(b.localId, 'en', { numeric: true }))
  if (!suttas.length) throw new Error('No SN/AN suttas found')
...
export const hooks: RecipeHooks<Parsed, Parsed> = {
  ...
  map(parsed, context) {
    ...
    for (const c of ['sn', 'an'] as const) {
      records.push({ id: work(c), record_type: 'resource', kind: 'textual.work', labels: [{ value: c === 'sn' ? 'Saṁyutta Nikāya' : 'Aṅguttara Nikāya', role: 'preferred', language: 'pli', script: 'Latn' }], extensions: { textual: { work_type: 'religious_text_collection' } } } as CorpusRecord)
    ...
    records.push({ id: PROVENANCE, record_type: 'provenance', source: artifact('sn'), source_reference: `SuttaCentral bilara-data published@${COMMIT}, SN and AN Sujato translation files`, activities: [{ type: 'acquisition', method: 'Pinned 3,227 GitHub source files with per-file SHA-256 manifest; upstream CRLF checksums are verified exactly after deterministic Git LF checkout reconstruction when needed', software: { name: 'mw:recipe:suttacentral:sn-an-sujato', version: '1' }, ended_at: '2026-08-27T00:00:00Z' }, { type: 'normalization', method: 'Identity; Bilara segment values are preserved exactly', software: { name: 'mw:recipe:suttacentral:sn-an-sujato', version: 'identity-1' } }] } as CorpusRecord)
  ...
  validate(records) { const suttas = records.filter((r) => r.record_type === 'resource' && r.kind === 'textual.passage' && r.extensions?.textual?.unit === 'sutta'); return suttas.length > 1000 ? [] : [`Expected more than 1,000 SN/AN suttas, got ${suttas.length}`] }
}
