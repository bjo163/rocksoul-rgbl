...
function canonicalize(value: unknown, stack: Set<object>): string {
  ...
  if (typeof value === 'number') {
    if (!Number.isFinite(value)) throw new TypeError('Canonical deterministic JSON rejects non-finite numbers')
    return encodedPrimitive(value)
  }
  if (Array.isArray(value)) {
    if (stack.has(value)) throw new TypeError('Canonical deterministic JSON rejects cyclic structures')
    stack.add(value)
    try {
      const encoded: string[] = []
      for (let index = 0; index < value.length; index += 1) {
        if (!(index in value)) throw new TypeError('Canonical deterministic JSON rejects sparse arrays')
  ...
  if (typeof value === 'object') {
    ...
    if (prototype !== Object.prototype && prototype !== null) throw new TypeError('Canonical deterministic JSON accepts only plain objects')
    if (Object.getOwnPropertySymbols(object).length > 0) throw new TypeError('Canonical deterministic JSON rejects symbol keys')
    if (stack.has(object)) throw new TypeError('Canonical deterministic JSON rejects cyclic structures')
    stack.add(object)
    try {
      const entries = Object.entries(object).sort(([left], [right]) => left < right ? -1 : left > right ? 1 : 0)
      return `{${entries.map(([key, entryValue]) => `${encodedPrimitive(key)}:${canonicalize(entryValue, stack)}`).join(',')}}`
...
export async function sha256Hex(value: string): Promise<string> {
  ...
  if (!subtle) throw new Error('SHA-256 deterministic IDs require Web Crypto SubtleCrypto support')
...
export async function deterministicCanonicalId(kind: string, payload: unknown): Promise<CanonicalId> {
  const digest = await sha256Hex(canonicalizeDeterministicJson(payload))
  return formatCanonicalId(kind, DETERMINISTIC_ID_RECIPE_VERSION, DETERMINISTIC_ID_HASH_ALGORITHM, digest)
}
...
function assertionObjectIdentityPayload(object: AssertionIdentityObject): Record<string, unknown> {
  ...
  if (hasEntity === hasValue) throw new TypeError('Assertion identity object must contain exactly one of entity or value')
  if (hasEntity) {
  ...
  if (literal.datatype !== undefined) {
    if (typeof literal.datatype !== 'string') throw new TypeError('Assertion literal datatype must be a string when present')
    payload.datatype = literal.datatype
  }
  if (literal.language !== undefined) {
    if (typeof literal.language !== 'string') throw new TypeError('Assertion literal language must be a string when present')
...
export function assertionIdentityPayload(input: AssertionIdentityInput): Record<string, unknown> {
  ...
  if (input.scope !== undefined) {
    assertAssertionScope(input.scope)
...
export function evidenceIdentityPayload(input: EvidenceIdentityInput): Record<string, unknown> {
  ...
  if (typeof input.relation !== 'string') throw new TypeError('Evidence relation must be a string')
  const payload: Record<string, unknown> = { record_type: 'evidence', relation: input.relation, target: input.target }
  if (input.selector !== undefined) {
    if (input.selector === null || Array.isArray(input.selector) || typeof input.selector !== 'object') throw new TypeError('Evidence selector must be a plain JSON object when present')
...
export function parseDeterministicId(value: string): ParsedDeterministicId {
  ...
  if (parsed.segments.length !== 3 || recipeVersion !== DETERMINISTIC_ID_RECIPE_VERSION || algorithm !== DETERMINISTIC_ID_HASH_ALGORITHM || !SHA256_HEX_PATTERN.test(digest)) {
    throw new TypeError("Invalid deterministic ID: expected 'mw:<kind>:v1:sha256:<64 lowercase hex characters>'")
  }
  return { kind: parsed.kind, recipeVersion, algorithm, digest }
}

export function isDeterministicId(value: unknown): value is CanonicalId {
  if (typeof value !== 'string') return false
  try { parseDeterministicId(value); return true } catch { return false }
}

export async function verifyDeterministicId(value: string, payload: unknown): Promise<boolean> {
  let parsed: ParsedDeterministicId
  try { parsed = parseDeterministicId(value) } catch { return false }
