...
async function main() {
  ...
  const { qualityReport, scoreDistribution } = await auditor.runAudit()

  console.log(JSON.stringify({
    success: true,
    totalWorks: qualityReport.totalWorks,
    averageScore: qualityReport.averageScore,
    minScore: scoreDistribution.min,
    maxScore: scoreDistribution.max,
    standardDeviation: scoreDistribution.standardDeviation,
    gradeBreakdown: qualityReport.gradeBreakdown,
    scoringDistributionCollapse: qualityReport.scoringDistributionCollapse
  }, null, 2))
}
