...
const roman = (value: string) => {
  ...
  for (const c of [...value.toUpperCase()].reverse()) { const n = m[c] ?? 0; total += n < previous ? -n : n; previous = n }
  return total
}
...
async function main() {
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    for (let j = i + 1; j < Math.min(i + 5, lines.length); j += 1) {
      ...
      if (candidate) { title = candidate.replace(/[._]+$/gu, ''); break }
    }
  ...
  for (let index = 0; index < starts.length; index += 1) {
    ...
    const passage = `mw:passage:ancient-egyptian:book-dead:${chapter.number}`
    const content = `mw:content:ancient-egyptian:book-dead:${chapter.number}:en-budge`
  ...
  await mkdir(path.join(CORE, 'resources'), { recursive: true }); await mkdir(path.join(CORE, 'provenance'), { recursive: true })
  const resource = `${DATASET}/data/core/resources/book-dead-budge.jsonl`; const provenance = `${DATASET}/data/core/provenance/book-dead-budge.jsonl`
  ...
  manifest.sourceSha256 = sourceHash; manifest.sourceByteSize = Buffer.byteLength(text)
  await writeFile(manifestPath, `${JSON.stringify(manifest, null, 2)}\n`, 'utf8')
  const checksumFiles = [resource, provenance, `${DATASET}/manifest.json`]
  const checksums = checksumFiles.map(file => `${sha256(await readFile(path.join(ROOT, file), 'utf8'))}  ${file}`)
