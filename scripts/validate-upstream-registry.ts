...
async function main() {
  ...
  if (!registry.traditions || typeof registry.traditions !== 'object') {
    fail('Missing traditions object')
  }
  ...
  for (const [traditionId, tradition] of Object.entries(registry.traditions)) {
    ...
    for (const endpoint of tradition.endpoints as Endpoint[]) {
      ...
      if (!id) {
        problems.push(`${traditionId}: endpoint missing id`)
      } else if (ids.has(id)) {
        problems.push(`duplicate endpoint id: ${id}`)
      } else {
        ids.add(id)
      }
      ...
      if (type === 'rest_api' && !endpoint.baseUrl) {
        problems.push(`${traditionId}/${id || '<unknown>'}: rest_api requires baseUrl`)
      }
      if (type === 'git_repo' && !endpoint.repoUrl) {
        problems.push(`${traditionId}/${id || '<unknown>'}: git_repo requires repoUrl`)
      }
      if (type === 'raw_archive' && !(endpoint.url || endpoint.baseUrl)) {
        problems.push(`${traditionId}/${id || '<unknown>'}: raw_archive requires url or baseUrl`)
      }
    }
  }
